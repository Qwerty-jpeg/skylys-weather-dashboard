        ...
        {data.nearby.map((place, index) => {
          ...
          return (
            <motion.div
              ...
              className="group relative overflow-hidden rounded-2xl bg-card border border-border/50 p-6 hover:shadow-lg hover:border-sky-200 dark:hover:border-sky-800 transition-all duration-300"
            >
              <div className="flex items-start justify-between mb-4">
                <div className="flex flex-col">
                  <span className="text-lg font-bold group-hover:text-sky-600 transition-colors">{place.name}</span>
                  <div className="flex items-center gap-1 text-xs text-muted-foreground mt-1">
                    <MapPin className="w-3 h-3" />
                    <span>{Math.round(place.distance)} km away</span>
                  </div>
                </div>
                <span className="text-4xl filter drop-shadow-sm">{getWeatherIcon(place.iconCode)}</span>
              </div>
              <div className="flex items-end justify-between">
                <div className="flex flex-col">
                  <span className="text-3xl font-bold tracking-tighter">{temp}°</span>
